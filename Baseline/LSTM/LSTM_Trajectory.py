...
def trainIters(encoder, n_iters, print_every=1000, plot_every=1, learning_rate=0.001):
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter % plot_every == 0:
            ...
            plot_loss_total = 0
            pltcount = 0
    return plot_losses,encoder.state_dict()
...
if TRAN_TAG:
    ...
    for epoch in (range(args.epochs)):
        ...
        for idx in idxs_users:
            plot_losses,local_model_weights = trainIters(copy.deepcopy(Prednet), 1, print_every=1)
            local_weights.append(copy.deepcopy(local_model_weights))
            local_losses.append(copy.deepcopy(plot_losses))

        # update global weights
        global_weights = average_weights(local_weights)

        # update global weights
        Prednet.load_state_dict(global_weights)
